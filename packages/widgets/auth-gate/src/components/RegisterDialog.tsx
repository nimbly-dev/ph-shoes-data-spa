...
export const RegisterDialog: React.FC<Props> = ({ open, onClose, onRegistered, onOpenLogin }) => {
  ...
  else if (!passwordComplexOK(password))
    fe.password = 'Password must include uppercase, lowercase, number, and special character';

  if (!confirm) fe.confirm = 'This field is required';
  else if (password !== confirm) fe.confirm = 'Passwords do not match';

  const showErr = (key: 'email' | 'password' | 'confirm') => (submitted || touched[key]) && (fe[key] || fieldErrors[key]);
